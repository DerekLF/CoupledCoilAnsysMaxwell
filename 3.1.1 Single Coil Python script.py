...
stack
    ...
    ICs=[]
    m3d.modeler.set_working_coordinate_system(cs_name)
    stackcnt = m3d.evaluate_expression(stack)
    for i in range(int(m3d[stack])//2):
        x=f"{inner_dia}/2"
        y=0
        z=f"-WireDiameter+(WirePitch*2)*{i}"
        IC=m3d.modeler.create_cylinder(
        orientation="z",
        origin=[x,y,z],
        radius="WireDiameter",
        height = "WirePitch+WireDiameter*2",
        num_sides = "CircleSegments",
        name=f"{name}_{i}_IC"
        )
        ICs.append(IC)
    if int(m3d[stack]) == 4:
        x=f"{inner_dia}/2+WirePitch*{turns}"
        y=0
        z=f"-WireDiameter+WirePitch"
        IC=m3d.modeler.create_cylinder(
        orientation="z",
        origin=[x,y,z],
        radius="WireDiameter",
...
create_IC("Main", "CS_Main", "Main_InnerDiameter","Main_Turns", "Main_Stack")
create_IC("Sec1", "CS_Sec1", "Sec_InnerDiameter","Sec_Turns", "Sec_Stack")
create_IC("Sec2", "CS_Sec2", "Sec_InnerDiameter","Sec_Turns", "Sec_Stack")
create_IC("Sec3", "CS_Sec3", "Sec_InnerDiameter","Sec_Turns", "Sec_Stack")
...
## 6. Calculate lead positions and generate leads
def create_lead (
name,
cs_name,
X_offset,
...
turns
    ...
    print(f"Assigning {name}_Coils leads")
    m3d.modeler.set_working_coordinate_system(cs_name)
    #bot is -Y=IN top is +Y=OUT
    
    #lead specific x0 and x1 variables
    lx0 = f"{inner_dia}/2+WirePitch*{turns}" #10.015
    lx1 = f"{boundary}/2-{inner_dia}/2-WirePitch*{turns}-{X_offset}"
    lwh = "WireDiameter"
    # why can I can't change "{inner_dia}/2+WirePitch*{turns}" for x0 is a really good question
    ly0 = "-WireDiameter"
    ly1 = "0"
    lz0 = "-WireDiameter/2"
    lz1 = f"-WireDiameter/2+WirePitch*({stack}-1)"
...
create_lead("Main", "CS_Main", "Main_X", "X_Boundary", "Main_InnerDiameter", "Main_Stack", "Main_Turns")
create_lead("Sec1", "CS_Sec1", "Sec1_X", "X_Boundary", "Sec_InnerDiameter", "Sec_Stack", "Sec_Turns")
create_lead("Sec2", "CS_Sec2", "Sec2_X*-1", "X_Boundary", "Sec_InnerDiameter", "Sec_Stack", "Sec_Turns")
create_lead("Sec3", "CS_Sec3", "Sec3_X*-1", "X_Boundary", "Sec_InnerDiameter", "Sec_Stack", "Sec_Turns")


## 7. Unite full model and assign material
def unite_coil(name, stack):
    objects = [
        f"{name}_L0",
        f"{name}_L1",
        f"{name}_L2",
...
unite_coil("Main", "Main_Stack")
assign_material("Main_Coil")

for i in range(1,4):
    unite_coil(f"Sec{i}", "Sec_Stack")
    assign_material(f"Sec{i}_Coil")

## 8. Create windings, add terminals and create intial solution setup.
...
turns
    ...
    m3d.modeler.set_working_coordinate_system(cs_name)
    #excitation specific variables, distance from center to boundary
    ex = f"{boundary}/2-{X_offset}"
    ey0 = "-WireDiameter/2"
    ey1 = "WireDiameter/2"
    ez0 = "0"
    ez1 = f"WirePitch*({stack}-1)"
    
    #Getting face data of the leads
    pos_in = [ex,ey0,ez0]
    pos_out = [ex,ey1,ez1]
    
    in_face = m3d.modeler.get_faceid_from_position(pos_in)
    out_face = m3d.modeler.get_faceid_from_position(pos_out)
    #assigning the Coil Terminals
    if not in_face:
        print(f"Error: No face found at {pos_in}. Check your coordinates!")
    else:
        print(f"Excitation position {pos_in} for {name}_lead_in ")
        term_in = m3d.assign_coil(
        assignment=[in_face],
        polarity = 'Positive',
        name = f"{name}_Terminal_In"
        )
    if not out_face:
        print(f"Error: No face found at {pos_out}. Check your coordinates!")
    else:
        print(f"Excitation position {pos_out} for {name}_lead_out ")
        term_out = m3d.assign_coil(
        assignment=[out_face],
        polarity = 'Negative',
...
assign_excitation("Main", "CS_Main", "Main_X", "X_Boundary", "Main_InnerDiameter", "Main_Stack", "Main_Turns")
assign_excitation("Sec1", "CS_Sec1", "Sec1_X", "X_Boundary", "Sec_InnerDiameter", "Sec_Stack", "Sec_Turns")
assign_excitation("Sec2", "CS_Sec2", "Sec2_X*-1", "X_Boundary", "Sec_InnerDiameter", "Sec_Stack", "Sec_Turns")
assign_excitation("Sec3", "CS_Sec3", "Sec3_X*-1", "X_Boundary", "Sec_InnerDiameter", "Sec_Stack", "Sec_Turns")